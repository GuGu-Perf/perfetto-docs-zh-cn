...
假设你想在 Android 设备或 Linux 服务器上开始记录长时间运行的 trace，然后终止你的 adb/ssh shell 并稍后返回停止 tracing 会话并收集 trace 文件。本页面向你展示如何执行此操作，同时确保 trace 保持完整。
...
使用 `tracebox` 或 `perfetto` 开始记录 trace。

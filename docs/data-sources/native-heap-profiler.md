...
Heapprofd 的转储在 UI 中显示为火焰图，单击相应的 slice 后显示。每个 slice 表示在该 slice 生命周期内收集的分配和调用栈的摘要。

![UI track 中的 heapprofd slice](/docs/images/profile-slice-malloc.png)
...
生成的 profile proto 包含数据的四个视图，对于每个 slice。

* **未释放的 malloc 大小**：在此调用堆栈上分配但未释放的字节数，在整个 slice 的持续时间内。
* **总 malloc 大小**：在此调用堆栈上分配（包括具有匹配释放的分配）的字节数，在整个 slice 的持续时间内。
...
生成的可视化显示多个 slice。单击每个 slice 显示该 slice 期间分配/释放的摘要。你可以拖动并选择多个连续的 slice 来汇总该时间窗口内的分配。

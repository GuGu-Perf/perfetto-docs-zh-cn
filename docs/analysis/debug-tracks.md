...
1. （对于 `slice` tracks）name（slice 的名称）列
...
1. 导航到"Timeline"视图，然后点击"Add debug track"来设置新的 debug track。选择"Slice Track"作为 Track 类型。
   ...
   注意：结果表中的列名不一定必须是 `name`、`ts` 或 `dur`。可以从下拉选择器中选择*语义*匹配但名称不同的列。
...
1. 导航到 Timeline 视图，然后点击"Add debug track"来设置新的 debug track。选择"Counter Track"作为 Track 类型，并选择语义匹配的感兴趣列名。
...
1. （可选）通过在"Pivot on"下拉菜单中选择列来创建透视 `counter` tracks。

...
PerfettoSQL 是一个强大的工具，用于交互式探索 trace。你可以编写任何想要的查询，结果立即可用。然而，这种灵活性给自动化和大规模分析带来了挑战。`SELECT` 语句的输出具有任意架构（列名和类型），可能会从一个查询更改为下一个查询。这使得构建消费此数据的通用工具、仪表板或回归检测系统变得困难，因为它们无法依赖稳定的数据结构。
...
这对于**跨 trace 分析**尤其强大。通过在数百或数千个 trace 上运行相同的汇总规范，你可以可靠地聚合结果，以跟踪一段时间内的性能 Metric，比较应用程序的不同版本，并自动检测回归。
...
我们可以定义一个 `TraceSummarySpec` 来计算此 Metric：
...
通常，你希望计算几个相关的 Metric，这些 Metric 共享相同的底层查询和维度。例如，对于给定进程，你可能想知道最小、最大和平均内存使用量。

与其为每个 Metric 编写单独的 `metric_spec`，这将涉及重复相同的 `query` 和 `dimensions` 块，你可以使用 [`TraceMetricV2TemplateSpec`](/protos/perfetto/trace_summary/v2_metric.proto)。这更简洁，更不容易出错，并且性能更高，因为底层查询只运行一次。
...
然后你可以运行它，请求任何或所有生成的 Metric，如下所示。
...
为了使 Metric 的自动化 Profile 和可视化更强大，你可以为 Metric 添加单位和极性（即，较高还是较低的值更好）。

这是通过在 `TraceMetricV2TemplateSpec` 中使用 `value_column_specs` 字段而不是简单的 `value_columns` 来完成的。这允许你为模板生成的每个 Metric 指定 `unit` 和 `polarity`。
...
这将为每个生成 Metric 的 `TraceMetricV2Spec` 添加指定的 `unit` 和 `polarity`，使输出更丰富，对自动化工具更有用。
...
同样，我们可以使用 `TraceMetricV2TemplateSpec` 从单个共享配置生成这些相关 Metric。
...
它的工作原理是采用一个 `base` 查询和一个或多个 `interval` 查询。结果仅包含 `base` 查询中与 _每个_ `interval` 查询的至少一行在时间上重叠的行。
...
你可以将键值元数据添加到汇总中，以为 Metric 提供上下文，例如设备型号或操作系统版本。这在分析多个 trace 时特别有用，因为它允许你基于此元数据分组或过滤结果。

元数据与你在同一运行中请求的任何 Metric 一起计算。
...
**2. 使用 Metric 和元数据运行汇总：**

运行汇总时，你指定要计算的 Metric 和用于元数据的查询。
...
每个 bundle 可以包含一起计算的一个或多个 Metric 的结果。使用 `TraceMetricV2TemplateSpec` 是创建 bundle 的最常见方式。从单个模板生成的所有 Metric 都自动放置在同一个 bundle 中，共享相同的 `specs` 和 `row` 结构。这非常高效，因为维度值通常是重复的，每行只写入一次。
...
Perfetto 以前有一个不同的系统来计算 Metric，通常称为"v1 Metric"。Trace 汇总是此系统的后继者，设计为更健壮且更易于使用。

以下是主要区别：

- **输出架构**：旧系统要求用户定义自己的输出 protobuf 架构。这很强大，但学习曲线陡峭，导致不一致、难以维护的输出。Trace 汇总使用单个、定义良好的输出 proto（`TraceSummary`），确保所有汇总的结构一致。
- **易于使用**：使用 trace 汇总，你不需要为输出编写或管理任何 `.proto` 文件。你只需要定义要计算的是*什么*数据（查询）及其*形状*（维度和值）。Perfetto 处理其余部分。
...
我们建议这仅用于 Perfetto "之上"的级别，即不用于配置如何从单个 trace 生成 Metric。
...
- **`metric_ids`**: 要计算的可选 Metric ID 列表。如果为 `None`，则计算规范中的所有 Metric。
- **`metadata_query_id`**: 用于 trace 范围元数据的查询的可选 ID。

TAB: Command-line shell

`trace_processor_shell` 通过 `summarize` 子命令暴露汇总功能。trace 文件是第一个位置参数；规范文件作为额外的位置参数跟在后面。

- **按 ID 运行特定 Metric**：使用 `--metrics-v2` 标志提供逗号分隔的 Metric ID 列表。
  ```bash
  trace_processor summarize \
    --metrics-v2 METRIC_ID_1,METRIC_ID_2 \
    TRACE_FILE YOUR_SPEC_FILE
  ```
- **运行规范中定义的所有 Metric**：使用关键字 `all`。
  ```bash
  trace_processor summarize \
    --metrics-v2 all \
    TRACE_FILE YOUR_SPEC_FILE
...
NOTE: 经典的 `--summary --summary-spec FILE --summary-metrics-v2 IDS
--summary-format FORMAT` 调用仍然受支持并生成相同的输出。参见 [Trace Processor → 子命令接口](/docs/reference/trace-processor-cli.md#subcommands) 了解新 CLI 的详细信息。
...
- **`query` (repeated
  ...
  定义可以被 Metric 引用或用于 trace 范围元数据的共享查询。
...
- **`id` (string)**: Metric 的唯一标识符。
- **`dimensions` (repeated string)**: 作为维度的列。
- **`value` (string)**: 包含 Metric 数值值的列。
- **`unit` (oneof)**: Metric 值的单位（例如 `TIME_NANOS`、`BYTES`）。也可以是 `custom_unit` 字符串。
...
定义用于从单个共享配置生成多个相关 Metric 的模板。当你有几个共享相同查询和维度的 Metric 时，这对于减少重复很有用。

使用模板会自动将生成的 Metric 捆绑到输出中的单个 [`TraceMetricV2Bundle`](/protos/perfetto/trace_summary/v2_metric.proto) 中。

- **`id_prefix` (string)**: 所有生成 Metric 的 ID 的前缀。
- **`dimensions` (repeated string)**: 所有 Metric 的共享维度。
- **`value_columns` (repeated string)**: 查询中的列列表。每列将使用 ID `<id_prefix>_<value_column>` 生成唯一的 Metric。
...
包含捆绑在一起的一个或多个 Metric 的结果。

- **`specs` (repeated `TraceMetricV2Spec`)**: bundle 中所有 Metric 的规范。
- **`row` (repeated `Row`)**: 每行包含维度值和该组维度的所有 Metric 值。

...
9. 服务向每个 producer 发送 [`StartDataSource`](/protos/perfetto/ipc/producer_port.proto#291) IPC 消息，针对 trace config 中配置的且在 producer 中存在的每个数据源（如果有）。
...
16. `SharedMemoryArbiter` 将带外发送 [`CommitDataRequest`](/protos/perfetto/ipc/producer_port.proto#44) 到服务，请求将共享内存缓冲区的某些 chunk 移动到最终 trace 缓冲区。
...
19. Consumer 向服务发送 [`FlushRequest`](/protos/perfetto/ipc/consumer_port.proto#61)，要求它提交 trace 缓冲区中所有正在传输的数据。
20. 服务向 tracing session 中涉及的所有 producers 发出 [`Flush`](/protos/perfetto/ipc/producer_port.proto#314) 请求。
21. Producer(s) 将 `Flush()` 所有它们的 `TraceWriter` 并向服务刷新请求回复。
22. 一旦服务收到来自所有 producers 的所有刷新请求的 ACK(或 [刷新超时](/protos/perfetto/ipc/consumer_port.proto#209) 已过期)，它就向消费者的 `FlushRequest` 回复。
23. Consumer 可选地发送 [`DisableTracing`](/protos/perfetto/ipc/consumer_port.proto#41) IPC 请求以停止 tracing 并冻结 trace 缓冲区的内容。
24. 服务将向每个 Producer 中的每个数据源广播 [`StopDataSource`](/protos/perfetto/ipc/producer_port.proto#299) 请求。
25. 此时 Consumer 发出 [`ReadBuffers`](/protos/perfetto/ipc/consumer_port.proto#48) IPC 请求（除非它之前在通过 trace config 的 `write_into_file` 字段启用 tracing 时向服务传递了文件描述符）。

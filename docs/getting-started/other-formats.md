...
- **重要说明：**
  ...
  - Perfetto 的数据模型要求持续时间事件（`B`/`E` 对和 `X` 事件）嵌套，这是规范要求的。重叠的、非嵌套事件是超出规范的，并且在本质上存在歧义；Perfetto 通过在单独的溢出 track 上布局它们来保持可见性，但结果可能与你的预期不符。有关详细信息和推荐修复方法（异步事件），请参阅 [issue #4280](https://github.com/google/perfetto/issues/4280)。
...
**描述：** Firefox Profiler JSON 格式主要用于 [Firefox Profiler](https://profiler.firefox.com/)，这是一个基于 Web 的性能 profiling 工具。该格式可以描述采样的 CPU 调用栈以及"markers"——类似于 slices 和 instants 的 Timeline 事件——Perfetto 会同时导入这两种数据。
...
- **使用跨平台 profiling 工具：** 一些工具原生发出 Firefox Profiler JSON，包括：
  - [**samply**](https://github.com/mstange/samply)，一个用于 Linux、macOS 和 Windows 的采样 CPU profiler，可以记录原生二进制文件并使用 `samply record --save-only -o profile.json` 将 profile 保存为 JSON 文件。
  - Python 标准库的 [`profiling.sampling`](https://docs.python.org/3.15/library/profiling.sampling.html) 模块（Python 3.15 新增），它有一个 `--gecko` 标志可以写入 Firefox Profiler JSON。
- **分析来自 Firefox 的 profiles：** 直接在 Firefox 浏览器中捕获的 profiles 通常同时包含 CPU 样本和丰富的 marker 数据（DOM 事件、IPC、GC 等）；Perfetto 可以同时导入这两种数据。
...
- **支持的格式变体：** Perfetto 支持 Firefox Profiler JSON 格式的两种变体：
  - **旧版格式：** 使用 `stringTable` 和带有 `schema` + `data` 数组的表（例如，`frameTable.schema` 和 `frameTable.data`）。这是历史上由 `perf script report gecko` 等工具生成的格式。此变体中不存在 Markers。
  - **预处理格式：** 使用 `stringArray` 和带有单独列数组的表（例如，`frameTable.func`、`funcTable.name`）。这是 Firefox Profiler 预处理输出使用的格式，由 `meta.preprocessedProfileVersion` 字段指示。如果存在 Markers，则会被导入。
  - 较新的顶层 `shared` 块（用于最新 Firefox 构建中跨线程去重字符串、帧和栈）也会被处理。
- **限制：**
  - Marker 的 `markerSchema` 定义和其他 UI 提示（自定义 track 颜色、图表/工具提示标签模板）不会被使用。
  - Profile 级别的元数据，如 `pages`、`counters` 和 `profilerOverhead`，不会被导入。

**如何生成：** 几种常见的路径可以产生 Perfetto 可导入的 Firefox Profiler JSON——从 Linux `perf` 或 Android `simpleperf` 转换原生 CPU profiles，使用 `samply` 记录，或使用标准库的 `profiling.sampling` 模块对 Python 进行采样。
...
1.  **使用 `perf record` 记录 profile：** 以 99Hz 采样 1 秒，使用 DWARF 调用图捕获特定命令/进程或系统范围的 CPU 样本，生成 `perf.data` 文件。
    ...
    如果你计划将此 profile 与 Perfetto trace 合并，请使用 `-k mono` 或 `-k boot` 录制，以使用与合并时间线兼容的时钟。有关更多详细信息，请参阅 [Perf 文本格式](#perf-textual-format) 部分。
...
[samply](https://github.com/mstange/samply) 是一个跨平台采样 CPU profiler，原生以 Firefox Profiler 格式记录。

1.  **安装 samply**，通过 `cargo install --locked samply` 或项目 README 中链接的预构建二进制文件。

2.  **记录 profile 并保存到文件**，使用 `--save-only` 参数，让 samply 跳过打开浏览器，仅写入 profile：
...
Python 标准库在 [`profiling.sampling`](https://docs.python.org/3.15/library/profiling.sampling.html) 模块中提供了采样 profiler。使用 `--gecko` 标志可以写入 Firefox Profiler JSON，包括 GIL、GC 的 markers，以及（使用 `--opcodes` 时）字节码级事件的 markers。

1.  **在采样器下运行脚本**（Python 3.15 或更新版本）：
...
- **samply（跨平台采样 profiler）：** [github.com/mstange/samply](https://github.com/mstange/samply)
- **Python 采样 profiler（3.15+）：** [`profiling.sampling`](https://docs.python.org/3.15/library/profiling.sampling.html)
...
1.  **使用 `perf record` 记录 profile：** 首先，使用 `perf record` 捕获 profiling 数据。这会创建一个 `perf.data` 文件。
    ...
    如果你计划将此 `perf script` 输出与 Perfetto trace 合并，请使用合并友好的时钟录制。默认的 perf 时钟可能导致合并文件时样本看起来与 Perfetto trace 偏移。添加 `-k mono` 使用 `CLOCK_MONOTONIC` 或 `-k boot` 使用 `CLOCK_BOOTTIME`，选择与你计划合并的 Perfetto trace 匹配的时钟：
...
## {#trace-archives} Trace 归档文件(.zip, .tar)
...
- 在某些特殊的、非 Fuchsia 用例中，需要一种二进制、紧凑且可流式传输的格式，与 Chrome JSON 的事件结构有相似之处。
  - 但是，对于 Fuchsia 生态系统之外的自定义 tracing 需求，通常建议生成 Perfetto 的[原生 TrackEvent protobuf 格式](/docs/reference/synthetic-track-event.md)，因为它功能更丰富且 Perfetto 工具的支持更好。
...
**描述：** `pprof` 格式是一种基于 protocol buffer 的格式，用于存储 CPU profile 数据。`pprof` 是一个用于可视化和 profile profiling 数据的工具。它读取 `profile.proto` 格式的 profiling 样本集合，并生成报告以可视化和帮助 profile 数据。它是为 [Go 编程语言的 pprof profiler](https://pkg.go.dev/runtime/pprof) 开发的，但后来被许多其他语言的其他 profilers 广泛采用（例如 Python、C++、Rust 等）。

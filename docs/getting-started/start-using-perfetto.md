...
graph TD
    ...
    B --> E2["<a href='#rust-developers'>Rust 开发者</a>"];
...
## {#rust-developers} Rust 开发者

如果你正在开发 Rust 应用程序，社区维护的 Perfetto Rust SDK 允许你对代码进行插桩、采集 trace 并与 Perfetto tracing 服务集成。它支持标准的 Rust `tracing` crate 以及直接的 Perfetto track event 宏。

- **如何为我的 Rust 应用添加 Perfetto tracing？** Rust SDK 提供 track event 宏、`#[tracefn]` proc macro 用于自动函数插桩，以及 `tracing-subscriber` Layer 用于将 Rust `tracing` 生态系统桥接到 Perfetto。

  - **教程**：[使用 Rust SDK 采集 Trace](/docs/getting-started/rust-sdk.md)
  - **参考**：[crates.io 上的 Rust SDK](https://crates.io/crates/perfetto-sdk)

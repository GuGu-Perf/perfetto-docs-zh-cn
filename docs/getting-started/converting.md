...
[TraceAttributes](/protos/perfetto/common/trace_attributes.proto) 消息允许你将此类 trace 级别的元数据直接附加到 `TracePacket`，无需 track 或时间戳。

Trace 级别元数据适用于：

- 记录生成 trace 的工具、库或流水线的版本。
- 附加 build ID 或 commit hash，以便将 trace 与生成它的代码关联。
- 记录采集 trace 的主机名、设备或环境。
- 任何属于特定 slice、counter 或 track 之外的其他生产者特定的 trace 信息。

要附加 trace 级别元数据：

1.  对于每条元数据，向 `TracePacket` 的 `trace_attributes.attribute` 字段添加一个 `Attribute`，设置其 `key` 以及 `string_value` 或 `long_value`。
2.  使用反向域名风格的前缀为你的 key 设置命名空间（例如 `com.example.myapp.build_id`），以免与其他生产者使用的 key 冲突。
3.  在 trace 的任何位置发出这些数据包。不需要 `timestamp`、`track_uuid` 或 `trusted_packet_sequence_id`。

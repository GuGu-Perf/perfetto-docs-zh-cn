...
本指南展示如何通过 `<iframe>` 将 Perfetto trace 查看器嵌入*你自己的*
...
- `url`（可选）：分享 URL。关于 `url` 和 `appStateHash` 如何启用分享，参见
  [深度链接](/docs/visualization/deep-linking-to-perfetto-ui.md)。
...
- 来自任何其他来源（例如你的生产域名）时，UI 会显示一个模态框：
  _"&lt;origin&gt; is trying to open a trace file. Do you trust the origin
  and want to proceed?"_，选项为 **No / Yes / Always trust**。"Always

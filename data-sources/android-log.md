...
这允许你查看与 trace 的其余部分时间同步的 Log event。当记录[长 trace]（/docs/concepts/config#long-traces）时，它允许你无限期地记录 Event log，而不管 Android Log 守护程序缓冲区大小如何（即，Log event 定期获取并复制到 trace 缓冲区中）。

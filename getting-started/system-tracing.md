# 使用 Perfetto 录制 system traces

在本指南中，你将学习如何：

- 在 Android 和 Linux 上录制 system-wide trace。
...
NOTE: 开箱即用，Perfetto 仅在 **Android** 和 **Linux** 上支持录制 system traces。虽然 trace 录制守护进程在 Windows 和 macOS 上工作，但没有与系统级数据源的集成，这意味着 traces 可能不太有用。

## 录制你的第一个 system trace

本节将指导你完成录制第一个 system-wide trace 的过程。根据你是想在 Android 上使用 GUI 录制、在 Android 上使用命令行录制还是在 Linux 上录制（仅命令行），有多种路径。
...
1. 首先导航到 [ui.perfetto.dev](https://ui.perfetto.dev)。这是 **Perfetto UI**，我们用于录制、profile 和可视化 traces 的多合一图形 UI；在本指南的其余部分，我们将大量使用它。
...
Perfetto 团队提供了一个名为 `record_android_trace` 的帮助脚本，用于在 Android 上从命令行录制 traces。这负责收集 trace、从服务器拉取它，甚至在 Perfetto UI 中打开它的大部分繁重工作：
...
上述命令应该会导致收集一个持续 10 秒的 trace：当它运行时，在 Android 设备上执行一些操作（例如打开应用程序、解锁手机等）。录制完成后，trace 将在浏览器窗口中自动在 Perfetto UI 中打开。
...
Perfetto 可以在 Linux 上捕获 system traces。支持所有基于 ftrace 的数据源和大多数其他基于 procfs / sysfs 的数据源。

由于 Perfetto 的 [service-based architecture](/docs/concepts/service-model.md)，为了捕获 trace，`traced` (session daemon) 和 `traced_probes` (probes 和 ftrace-interop daemon) 需要正在运行。根据 Perfetto v16，`tracebox` 二进制文件将你需要的所有二进制文件捆绑在一个静态链接的可执行文件中（有点像 `toybox` 或 `busybox`），这使得在不同机器上复制和运行变得容易。
...
要捕获 trace，你需要将配置文件传递给下载的 `tracebox` 二进制文件。我们在 [/test/configs/](/test/configs/) 目录中有一些示例配置文件。
假设你想捕获带有 scheduling 信息的 trace。你可以通过下载配置文件来执行此操作
```bash
...
```
scheduling 信息是使用 ftrace 捕获的，因此你可能需要以 root 权限启动 `tracebox`。

</tabs?>

## 查看你的第一个 trace

我们现在可以通过使用基于 Web 的 trace 可视化工具：Perfetto UI 来直观地探索捕获的 trace。
...
上面的录制说明都应该导致 trace 在浏览器中自动打开。但是，如果由于某种原因它们不起作用（最可能的是如果你通过 SSH 运行命令），你也可以手动打开 traces：

1. 在浏览器中导航到 [ui.perfetto.dev](https://ui.perfetto.dev)。
2. 点击左侧菜单上的 **Open trace file**，并加载捕获的 traces 或简单地将你的 trace 拖放到 Perfetto UI 中。
...
3.  你现在可以执行查询。例如，要查看 trace 中捕获的所有进程，请运行以下查询（你可以使用 Ctrl/Cmd + Enter 作为快捷方式）：
...
现在你已经录制并分析了你的第一个 system trace，你可以探索更多主题：

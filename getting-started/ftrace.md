...
在本地内核上试验并希望简单的 track 可视化而不接触 perfetto 代码的人可以浏览[第 C 部分][section-c-link]。
...
本页面分为三个部分：
- [第 A 部分][section-a-link]：涵盖创建一个带有静态 tracepoint 的示例内核模块，使用 perfetto 记录事件，以及在 perfetto UI 中查看基本事件数据。
- [第 B 部分][section-b-link](高级)：涵盖通过修改 perfetto 源代码为 tracepoint 添加专用解析。这让你可以充分利用将事件转换为结构化的 SQL 表和 UI tracks。
- [第 C 部分][section-c-link]：作为上述内容的替代，描述了构建 tracepoints 的约定，以便 perfetto 可以自动将它们转换为 slices/instants/counters，**而无需修改 perfetto 源代码**。
...
请参阅[系统 tracing 页面]（/docs/getting-started/system-tracing.md）以设置 tracebox。对于此示例，我们将使用刚刚创建的配置文件从命令行记录 trace：
```bash
...
我们现在可以在 perfetto UI 中探索记录的 trace。导航到 https://ui.perfetto.dev 并将文件拖放到窗口中（或按 `Ctrl+O` 打开文件对话框）。
...
## 第 B 部分：将新的 tracepoints 与 perfetto 集成
...
使用你的更改在本地重建 `tracebox` 并重新记录 trace。
...
重建本地 UI(`ui/run-dev-server`)。打开重新记录的 trace(使用具有事件编译时知识的 tracebox)。你现在应该看到一个专用 UI track 显示你事件的数据。
...
## 第 C 部分：无需修改 perfetto 代码的简单 slice/counter 可视化(内核 track 事件)
...
我们可以使用以下配置记录 trace(在撰写本文时，`denser_generic_event_encoding` 是必要的，但可能会成为默认值)：
...
使用前面示例中的配置记录时的结果可视化。所有 slice 堆栈都在进程级别聚合：
...
使用前面示例中的配置记录时的结果可视化。Counter 增量归因于执行 tracepoint 的 CPU(由于使用 `smp_processor_id()` 作为 CPU 索引，但如果对 tracepoint 更有意义，我们可以同样使用静态索引)：

...
> NOTE: 要从 Android 上的 Chrome 记录 traces，请遵循[记录 Android 系统 traces 的说明]（/docs/getting-started/system-tracing.md）并启用 Chrome Probe。如果你使用的是 [Android 的用户构建版本](https://source.android.com/docs/setup/build/building#lunch)，你必须通过将 `chrome://flags#enable-perfetto-system-tracing` 切换为"已启用"并重启 Chrome 来启用与系统 Perfetto 的集成。

## 手动录制 trace

> NOTE: 如果需要自动 trace 收集，请遵循 [crossbench 说明](#使用-crossbench-自动化记录-trace)。

1. 导航到 [ui.perfetto.dev](https://ui.perfetto.dev/) 并从左侧菜单中选择 [**"记录新 trace"**](https://ui.perfetto.dev/#!/record)。
...
5. 现在你可以开始 trace 记录了。准备好时按 **"开始记录"**按钮。
...
1. 按照[手动过程]（#手动记录-trace）中的步骤 1-4 创建 trace 配置。
